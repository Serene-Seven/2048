...
public class GameView extends GridLayout implements View.OnTouchListener {
    ...
    private void judgeDirection(int offsetX, int offsetY) {
        ...
        if (flagNormal && !flagSuper) {
            ...
            builder.setTitle("上帝模式：请输入你想要生成的数字（最高1024）")
                    .setView(et)
    ...
    private int checkNums() {
        getBlanks();
        // 如果没有空的数字格子，判断所有格子与之邻近的格子有没有数字相等的
        if (mBlanks.size() == 0) {
            for (int i = 0; i < mGameLines; i++) {
                for (int j = 0; j < mGameLines; j++) {
                    if (i < mGameLines - 1) {
                            ...
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface arg0,
                                    ...
                                    if (mTarget == 1024) {
                                        ...
                                        mTarget = 8192;
                                        MainActivity.getGameActivity().setGoal(8192);
                                    }
                                    editor.apply();
    ...
    /**
     * 判断是否移动过(是否需要新增Item)
     * 不能根据是否滑动过判断，因为滑动过后前后不变的不能新增Item
     * @return 是否移动
     */
    ...
    private void swipeUp() {
        for (int i = 0; i < mGameLines; i++) {
            for (int j = 0; j < mGameLines; j++) {
                ...
                if (currentNum != 0) {
                    if (mKeyItemNum == -1) {
                        ...
                        // 下一个如果跟上一个是相等的就相加，新矩阵保存新产生的值；否则下一个变成新的key，上一个被保存
            ...
            // 当一整列计算完毕时，最后一个可能正好是key，也要记得放进新的里面。
            if (mKeyItemNum != -1) {
                mCalList.add(mKeyItemNum);
            }
            // 改变Item值
